...
from utils.llm_mapping import gerar_mapeamento_llms_por_config, obter_config_por_nome
...
def export_import_interface():
    ...
    mapeamento_llms = gerar_mapeamento_llms_por_config()
    ...
    agentes_json = [
        {
            ...
            "tools": a.tools,
            "llm_name": mapeamento_llms.get(json.dumps(a.llm_config, sort_keys=True), "desconhecido")
        }
        for a in agentes
    ]
        ...
        try:
            ...
            for agent in imported_agents:
                if agent.get("agent_id") in agentes_existentes_ids:
                    st.warning(f"⚠️ Agente duplicado ignorado: {agent.get('agent_id')}")
                    continue

                llm_name = agent.get("llm_name")
                llm_config = obter_config_por_nome(llm_name)

                if not llm_config:
                    st.warning(f"❌ Modelo LLM '{llm_name}' não encontrado. Agente '{agent.get('name')}' não será importado.")
                    continue
